...
export const getInvoiceInventory = async (req, res) => {
  try {
    const allProducts = await inventoryModel.find().select(['StockQuantity', 'ItemName', 'PurchasePrice']);

    res.status(200).json({
        data: allProducts,
        success: true,
        message: 'Successfully fetched invoice items',
    });
  } catch (error) {
...
export const deleteProductFromInventory = async (req, res) => {
  try {
    ...
    const dataInstance = await inventoryModel.deleteMany({
      _id: { $in: selectedIds },
    });
...
export const adjustStockById = async (req, res) => {
  try {
    ...
    if (formValues.type === "add") {
      finalStock =
        Number(dataInstance.StockQuantity) +
        Number(formValues.adjustStockQuantity);
    } else {
      finalStock =
        Number(dataInstance.StockQuantity) -
        Number(formValues.adjustStockQuantity);
